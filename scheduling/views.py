...
def schedule_summary_by_term(request, term):
    ...
    count_by_course = Course.objects.filter(schedule__term=term, subject__in=subject_list).annotate(num_sections=Count("schedule")).order_by("-num_sections")
    count_by_instructor = Instructor.objects.filter(schedule__term=term).annotate(num_sections=Count("schedule")).order_by("-num_sections")
